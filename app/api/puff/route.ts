...
export async function GET(req: NextRequest) {
  ...
  console.log("xPaymentResponse", xPaymentResponse);
  if (!xPaymentResponse) {
    console.error("Missing payment response header");
  ...
  console.log("xPaymentResponseDecoded", xPaymentResponseDecoded);
  ...
  const sendResult = await x420SmartAccount.transfer({
    ...
    to: xPaymentResponseDecoded.payer,
